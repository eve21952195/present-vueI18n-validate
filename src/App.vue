...
// 測試自製規則
defineRule('biger', (value, [limit], ctx) =>{
    ...
    if(value.length < limit) {
      console.log('ctx', ctx);
        return t('validation.messages.biger', {field: ctx.label,limit: limit});
    }
    ...
    <div class="wrapper">
      <nav>
        <RouterLink
          to="/"
          class="btn"
          :class="{ active: current === 'deposit' }"
          @click="changePage('deposit')"
          >{{ t("tradingPage.deposit.tab") }}</RouterLink
        >
        <RouterLink
          to="/withdraw"
          class="btn"
          :class="{ active: current === 'withdraw' }"
          @click="changePage('withdraw')"
          >{{ t("tradingPage.withdraw.tab") }}</RouterLink
        >
      </nav>
